...
traceback(const std::string &seq1, const std::string &seq2,
  ...
  while (current_score > 0) {
    ...
    int diag_score = (diag_i >= 0 && diag_j >= 0) ? H[idx(diag_i, diag_j)] : 0;
...
SWResult smith_waterman(const std::string &seq1, const std::string &seq2,
                        int match_score, int mismatch_score, int gap_penalty, bool CUDA=false) {
    ...
    if (CUDA) {
      H = cuda_sw_create_scoring_matrix(seq1, seq2, match_score,
                                        mismatch_score, gap_penalty,
                                        rows, cols);
    } else {
      H = cpu_sw_create_scoring_matrix(seq1, seq2, match_score,
                                       mismatch_score, gap_penalty,
                                       rows, cols);
    }
    ...
    auto [aligned1, aligned2] = traceback(
        seq1, seq2, H, rows, cols, {max_i, max_j},
        max_val, match_score, mismatch_score, gap_penalty
    );
...
//     SWResult result_cpu = smith_waterman(seq1, seq2, match_score, mismatch_score, gap_penalty, false);
...
//     SWResult result_cuda = smith_waterman(seq1, seq2, match_score, mismatch_score, gap_penalty, true);
