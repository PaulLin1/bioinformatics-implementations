...
__global__ void sw_compute_diagonal_kernel(
    ...
    int* __restrict__ H, int rows, int cols,
    ...
    int val = diag_score;
    if (up_score   > val) val = up_score;
    if (left_score > val) val = left_score;
    if (val < 0) val = 0;
...
std::vector<int> cuda_sw_create_scoring_matrix(const std::string& seq1,
    ...
    */
    ...
    for (int k = 2; k <= rows + cols - 2; ++k) {
        ...
        sw_compute_diagonal_kernel<<<blocks, TPB>>>(
            d_seq1, d_seq2, d_H, rows, cols,
            k, i_start, diag_len, match_score, mismatch_score, gap_penalty
        );
    ...
    CUDA_CHECK(cudaMemcpy(H.data(), d_H, rows * cols * sizeof(int), cudaMemcpyDeviceToHost));
